...
public class CraftingMenu extends Menu {
    ...
    public CraftingMenu(Claim claim, Menu previous) {
        ...
        this.missions = ClaimCore.getInstance().getMissionsManager().getMissions(this.claim.getMissionsTier(), MissionType.CRAFTING);
        ...
        for(int i = 0; i < this.missionsSlots.size();) {
            ...
            if(!this.claim.hasCompleted(mission)) {
                if (!this.claim.hasCompleted(mission.getRequiredMission())) {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Crafting.Item.Locked"), textPlaceholders);
                    ...
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Crafting.Item.Select"), textPlaceholders.set("%progress%", finalProgress));
                } else {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Crafting.Item.Available"), textPlaceholders);
                }
            } else {
                builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Crafting.Item.Complete"));
            }
