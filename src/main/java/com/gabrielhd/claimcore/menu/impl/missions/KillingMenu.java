...
public class KillingMenu extends Menu {
    ...
    public KillingMenu(Claim claim, Menu previous) {
        ...
        this.missions = ClaimCore.getInstance().getMissionsManager().getMissions(this.claim.getMissionsTier(), MissionType.KILLING);
        ...
        for(int i = 0; i < this.missionsSlots.size();) {
            ...
            if(!this.claim.hasCompleted(mission)) {
                if (!this.claim.hasCompleted(mission.getRequiredMission())) {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Killing.Item.Locked"), textPlaceholders);
                    ...
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Killing.Item.Select"), textPlaceholders.set("%progress%", finalProgress));
                } else {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Killing.Item.Available"), textPlaceholders);
                }
            } else {
                builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Killing.Item.Complete"));
            }
