...
public class MiningMenu extends Menu {
    ...
    public MiningMenu(Claim claim, Menu previous) {
        ...
        this.missions = ClaimCore.getInstance().getMissionsManager().getMissions(this.claim.getMissionsTier(), MissionType.MINING);
        ...
        for(int i = 0; i < this.missionsSlots.size();) {
            ...
            if(!this.claim.hasCompleted(mission)) {
                if (!this.claim.hasCompleted(mission.getRequiredMission())) {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Mining.Item.Locked"), textPlaceholders);
                    ...
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Mining.Item.Select"), textPlaceholders.set("%progress%", finalProgress));
                } else {
                    builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Mining.Item.Available"), textPlaceholders);
                }
            } else {
                builder = getItemStack("Missions.yml", missionsConfig.getConfigurationSection("Missions.Mining.Item.Complete"));
            }
